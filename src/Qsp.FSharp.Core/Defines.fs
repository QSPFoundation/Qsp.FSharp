...
let functions =
    ...
    [
        ...
        let dscr =
            [
                "Возвращает версию интерпретатора в формате \"X.Y.Z\" Player."
                ""
                "С версии 5.8 доступна перегрузка `string` типа со значением:"
                ""
                "* `'platform'` — возвращает платформу, на которой запущена игра"
            ] |> String.concat "\n"
        let overloads =
            [
                [||], ()
...
let procedures =
    [
        ...
        let dscr =
            [
                "`ADDOBJ [$название],[$путь к файлу изображения],[#индекс]` или `ADD OBJ [$название],[$путь к файлу изображения],[#индекс]` - добавление предмета с названием [$название] и изображением [$путь к файлу изображения] в инвентарь на место с номером [#индекс]."
                ""
